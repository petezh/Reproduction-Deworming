...
This report summarizes a reproduction attempt for **`r raw_json$paper[['title']]`** by `r raw_json$paper[['authors']]`. It is based on a [submission](`r ssrp_link`) on the [Social Science Reproduction Platform](https://www.socialsciencereproduction.org/) (SSRP). The improvements follow the [guidelines](https://bitss.github.io/ACRE/improvements.html) outlined by the [Accelerating Computational Reproducibility in Economics](https://www.bitss.org/ecosystem/acre/) (ACRE) project. The reproducer implemented and suggested improvements to the paper's reproducibility.
...
if (num_packages > 0) { # if exists
  ...
  df[,3] <- paste0("[Link](", df[,3], ")") # convert to markdown
